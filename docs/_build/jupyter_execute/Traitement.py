...
# Avant de démarrer l'exécution de ces cellules, vous devez avoir déjà créé dans **pgAdmin** le schéma dans lequel les données du classeur Excel seront chargées et traitées. Le nom de ce schema doit commencé par **`"socio_hab_"`**.
# Il est en outre nécessaire de fermer tous les fichiers Excel qui seront utilisés pendant ce traitement.

# Voici ci-dessous l'architecture de dossier à respecter : 
# ![](DossierDAT.PNG)

# Le script commence par une cellule qui importe les bibliothèques nécessaires au bon déroulement du traitement et du transfert des données. L'avant-dernière ligne similaire à la ligne de code suivante représente l'emplacement du fichier **`fonctions.py`**, il doit donc être ajusté en fonction de son emplacement sur votre ordinateur.
...
# Dans l'exemple ci-dessus , mon fichier fonction se trouve dans dans le dossier **"BDD"** dont le chemin est `"C:/Users/malaka/Documents/prog/BDD"`. La cellule après celle des imports de librairies, permet ed créer un dossier temporaire nommé **"DossierFichiersbrutesCsv"** qui sera supprimer à la fin du programme.
# 
...
# - `suffix_nom_schema` = le nom qui est donné au shéma dans lequel va être versé ces données, sans la partie 'sociohab'. `**Doit être en minuscule**`
